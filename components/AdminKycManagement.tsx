...
export default function AdminKycManagement() {
  ...
  return (
    <div className="min-w-0 space-y-6">
      <header className="rounded-3xl border border-slate-200 bg-white p-4 sm:p-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="break-words text-xl font-extrabold text-slate-900">Contrôle structuré des identités</h1>
      ...
      <section className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 sm:p-6">
        <div className="relative mb-5 max-w-sm">
          ...
          <input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nom, e-mail ou référence" className="min-h-11 w-full rounded-xl border bg-slate-50 py-2 pl-9 pr-3 text-xs" />
        </div>
            ...
            return (
              <article key={application.id} className="min-w-0 rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                ...
                <div className="mt-3 min-w-0 rounded-xl bg-white p-2.5 text-[10px]">
                  ...
                  <p className="mt-1 break-all font-mono font-bold text-slate-800">{application.id}</p>
                  <p className="mt-0.5 break-words text-slate-500">{application.submittedAt}</p>
                </div>
                <span className={`mt-3 inline-flex max-w-full items-center gap-1 break-words rounded-full px-2.5 py-1 text-[10px] font-bold ${status === 'approved' ? 'bg-emerald-100 text-emerald-800' : status === 'rejected' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'}`}>
                  {status === 'approved' ? <CheckCircle2 className="h-3 w-3 shrink-0" /> : status === 'rejected' ? <XCircle className="h-3 w-3 shrink-0" /> : <Clock className="h-3 w-3 shrink-0" />}
                  {STATUS_LABELS[status]}
                </span>
                <button type="button" onClick={() => openApplication(application)} className="mt-4 inline-flex min-h-11 w-full items-center justify-center gap-1 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-bold text-white">
        ...
        {!applications.length && <p className="py-10 text-center text-sm text-slate-500">Aucun dossier d’identité.</p>}
      </section>

      {selected && <Dialog open onClose={() => setSelected(null)} ariaLabelledBy="admin-kyc-dialog-title">
        <DialogBackdrop className="fixed inset-0 z-50 flex items-end justify-center overflow-hidden bg-slate-950/75 pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)] pt-[env(safe-area-inset-top)] backdrop-blur-sm sm:items-center sm:px-4 sm:pb-4 sm:pt-[max(1rem,env(safe-area-inset-top))]">
        <DialogPanel className="max-h-dvh w-full min-w-0 max-w-6xl space-y-6 overflow-y-auto overscroll-contain rounded-t-3xl bg-white p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-2xl sm:max-h-[calc(100dvh-2rem)] sm:rounded-3xl sm:p-6">
          <header className="flex min-w-0 items-start justify-between gap-3 border-b pb-4">
            <div className="min-w-0"><h2 id="admin-kyc-dialog-title" className="break-words text-lg font-extrabold">{selected.firstName} {selected.lastName}</h2><p className="break-all font-mono text-xs text-slate-500">{selected.id}</p></div>
            <button type="button" onClick={() => setSelected(null)} aria-label="Fermer" className="flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded-xl hover:bg-slate-100"><X className="h-5 w-5" /></button>
          </header>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="space-y-3">
              <div className="flex flex-wrap gap-2">
                ...
                ] as const).map(([key, label]) => <button key={key} type="button" onClick={() => setDocumentKey(key)} className={`min-h-11 rounded-xl px-3 py-2 text-xs font-bold ${documentKey === key ? 'bg-slate-900 text-white' : 'bg-slate-100'}`}>{label}</button>)}
              </div>
              <div className="flex h-56 items-center justify-center overflow-hidden rounded-2xl bg-slate-950 sm:h-80">
                {selected.documents[documentKey] ? selected.documents[documentKey].includes('.pdf?')
                  ? <a href={selected.documents[documentKey]} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center rounded-xl bg-white px-4 py-3 text-xs font-bold">Ouvrir le PDF privé</a>
            ...
            <div className="space-y-4 text-xs">
              ...
              <dl className="divide-y overflow-hidden rounded-2xl border">
                ...
                ].map(([label, value]) => <div key={label} className="flex min-w-0 flex-col gap-1 p-3 min-[360px]:flex-row min-[360px]:justify-between min-[360px]:gap-4"><dt className="text-slate-500">{label}</dt><dd className="min-w-0 break-words font-bold min-[360px]:text-right">{value}</dd></div>)}
              </dl>
            </div>
          </div>
          ...
          {selected.workflowStatus === 'under_review' && <div className="grid gap-6 border-t pt-6 lg:grid-cols-2">
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-2 min-[360px]:grid-cols-2">
                {REQUEST_ITEMS.map(([key, label]) => <label key={key} className="flex min-h-11 items-center gap-2 rounded-lg border p-2 text-xs"><input type="checkbox" checked={requested.includes(key)} onChange={() => toggleRequested(key)} />{label}</label>)}
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <button type="button" disabled={busy} onClick={() => void run(async () => { await updateKYCChecklist(selected.id, checklist); await approveKYCApplication(selected.id, checklist.internalComments); })} className="min-h-11 w-full rounded-xl bg-emerald-600 px-3 py-3 font-bold text-white disabled:opacity-50">Approuver et créer le compte</button>
                <button type="button" disabled={busy || !note.trim()} onClick={() => void run(async () => { await updateKYCChecklist(selected.id, checklist); await rejectKYCApplication(selected.id, note, reasonCode); })} className="min-h-11 w-full rounded-xl bg-rose-600 px-3 py-3 font-bold text-white disabled:opacity-50">Rejeter</button>
              </div>
            </section>
          </div>}
