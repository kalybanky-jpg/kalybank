...
const COPY = {
  fr: { title: 'Suivi de mon identité', submitted: 'Date de soumission', next: 'Prochaine action', waiting: 'Aucune action requise. {bankName} examine votre dossier.', correct: 'Corrigez uniquement les éléments signalés puis resoumettez le même dossier.', done: 'Votre identité est confirmée et votre compte interne est ouvert.', edit: 'Corriger mon dossier', noFile: 'Aucun dossier KYC soumis.', start: 'Commencer la vérification', resume: 'Reprendre la vérification', inProgress: 'Dossier en cours : étape {step} sur {total}', due: 'Échéance' },
  en: { title: 'My identity status', submitted: 'Submission date', next: 'Next action', waiting: 'No action required. {bankName} is reviewing your file.', correct: 'Correct only the flagged items, then resubmit the same file.', done: 'Your identity is confirmed and your internal account is open.', edit: 'Correct my file', noFile: 'No KYC file submitted.', start: 'Start verification', resume: 'Resume verification', inProgress: 'Application in progress: step {step} of {total}', due: 'Due date' },
...
export default function UserKycStatusView() {
  ...
  const [draftInfo, setDraftInfo] = React.useState<{ currentStep: number; totalSteps: number } | null>(null);
    ...
    void (async () => {
      try {
        ...
        const { data: draft } = await supabase
          ...
          .eq('owner_id', user.id)
          .maybeSingle();
        if (draft && typeof draft.current_step === 'number') {
          if (active) setDraftInfo({ currentStep: draft.current_step + 1, totalSteps: 9 });
          return;
        }
        if (typeof window !== 'undefined') {
          const local = localStorage.getItem(`monalyz_kyc_draft_${user.id}`);
          if (local) {
            const parsed = JSON.parse(local);
            if (typeof parsed.stepIndex === 'number' && active) {
              setDraftInfo({ currentStep: parsed.stepIndex + 1, totalSteps: 9 });
            }
          }
        }
      } catch {}
  ...
  if (!application) return (
    <div className="mx-auto max-w-3xl min-w-0 p-4 pb-[calc(6rem+env(safe-area-inset-bottom))] sm:p-6 sm:pb-[calc(6rem+env(safe-area-inset-bottom))]">
      <section className="min-w-0 rounded-3xl border bg-white p-4 text-center sm:p-8">
        ...
        <p className="mt-2 text-sm text-slate-500">{draftInfo ? copy.inProgress.replace('{step}', String(draftInfo.currentStep)).replace('{total}', String(draftInfo.totalSteps)) : copy.noFile}</p>
        <Link href="/onboarding" className="mt-6 inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white sm:w-auto">{draftInfo ? copy.resume : copy.start}<ArrowRight className="h-4 w-4 shrink-0" /></Link>
      </section>
    </div>
  );
    ...
    <div className="mx-auto max-w-4xl min-w-0 p-4 pb-[calc(6rem+env(safe-area-inset-bottom))] sm:p-6 sm:pb-[calc(6rem+env(safe-area-inset-bottom))]">
      ...
      <section className="min-w-0 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        {needsAction && <>
          ...
          <Link href="/onboarding" className="mt-5 inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white sm:w-auto">{copy.edit}<ArrowRight className="h-4 w-4 shrink-0" /></Link>
        </>}
      </section>
    </div>
  );
